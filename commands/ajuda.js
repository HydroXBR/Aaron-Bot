...
exports.run = async (client, message, args) => {
  ...
  let embed = new MessageEmbed()
    ...
    **VEJA MEUS COMANDOS POR CATEGORIA:**
    > :briefcase: **ADMINISTRAÇÃO, INFORMAÇÕES DO SERVIDOR:** Digite \`${pp}ajuda adm\`
    > :soccer: **DIVERSÃO, ETC:** Digite \`${pp}ajuda diversão\` ou \`${pp}ajuda fun\`
    > :newspaper: **INFORMAÇÕES, PESQUISA:** Digite \`${pp}ajuda info\`
    > :dollar: **ECONOMIA** Digite \`${pp}ajuda eco\` ou \`${pp}ajuda economy\`

    **PARA VER AJUDA SOBRE UM ÚNICO COMANDO:**
    > Digite \`${pp}ajuda comando\`
  ...
  let brembed = new MessageEmbed()
    ...
    .addField(`${pp}stonks <texto>`, `Crie um meme stonks`)
    .addField(`${pp}notstonks <texto>`, `Crie um meme not stonks`)
...
if (/adm|administraç(ã|a)o/i.test(message.content)) {
    admembed.setFooter(`Espero ter ajudado :)`)
    message.inlineReply(admembed)
  } else if (/diver(s|ç|c)(a|ã)o|fun|brinks|bri(m|n)cadeira/i.test(message.content)) {
    brembed.setFooter(`Espero ter ajudado :)`)
    message.inlineReply(brembed)
  } else if (/i(m|n)fo|txt|texto/i.test(message.content) && !/serv/i.test(message.content)) {
    txembed.setFooter(`Espero ter ajudado :)`)
    message.inlineReply(txembed)
  } else if (/eco(nomy)?/gmi.test(message.content)) {
    message.inlineReply(economyembed)
  } else if (args[0]) {
    try{
      let command = args[0]
      for (var i = 0; i < json.commands.length; i++){
        if (json.commands[i].name == command){
          let info = json.commands[i].info
          let similares = json.commands[i].similar
          let ddembed = new MessageEmbed()
            .setTitle(`Ajuda - Comando ${command}`)
            .setColor(`#A020F0`)
          ...
          message.inlineReply(ddembed)
        }
      }
    }catch(error){
      message.inlineReply(`❌ | Ops, ${message.author}! Você me pediu ajuda para um comando, mas parece que o comando não existe! Verifique a ortografia e se o comando existe na lista, digitando \`${pp}ajuda\` :wink:`)
    }
  } else {

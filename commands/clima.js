...
exports.run = async (client, message, args) => {
  ...
  if(!args[0]) return message.inlineReply('❌ | Você não especificou um local. Tente novamente :)')

  if(result === undefined || result.length === 0) return message.inlineReply('❌ | Você enviou uma localização inválida. Por favor, tente novamente e verifique a escrita. Obrigado!');
